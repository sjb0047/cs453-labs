...
1. What makes this API more "REST-like" than the previous HTTP/JSON lab?
   This API is more REST-like due to the way actions are organized around a resource. Other methods are GET, POST, PUT, and DELETE to perform different actions on that resource.
2. What is the purpose of a route parameter such as `/items/:id`?
   A route parameter allows part of the URL to act as a variable. In `/items/:id`, the `id` identifies which specific item the request should use.
3. Why should `POST`, `PUT`, and `DELETE` use different HTTP methods?
   They should use different HTTP methods because they represent different actions. POST creates new item, PUT updates  existing item, DELETE removes item.
4. What is the difference between a `400` error and a `404` error?
   A 400 error means the client sent invalid or missing data. A 404 error means the requested route or item could not be found.
5. How does the OpenAPI file relate to your Express server code?
   The OpenAPI file describes the API routes, expected request bodies, responses, and status codes.
